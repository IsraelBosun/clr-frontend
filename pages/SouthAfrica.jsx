...
import SouthAfricaTop5 from "./components/SouthAfrica/southAfricaTop5";
import SouthAfricaMissedRepayment from "./components/SouthAfrica/southAfricaMissedRepayments";
import SouthAfricaStage2 from "./components/SouthAfrica/southAfricaStage2";
import SouthAfricaSector from "./components/SouthAfrica/southAfricaSector";    
...
const SouthAfricaScreen = () => {
  ...
  const loadPersistedData = async () => {
    const querySnapshot = await getDocs(collection(db, "southAfricaData"));
  ...
  const handleDataLoaded = async (data) => {
    ...
    const relevantData = {
      ...
      percentage_of_top5: data.percentage_of_top5,
      ppl: data.ppl,
      wpl: data.wpl,
      npl: data.npl,
      mrr: data.mrr,
    ...
    await setDoc(doc(db, "southAfricaData", timestamp), relevantData);
  };
  ...
  {/* // md:w-1/2 lg:w-3/4 xl:w-full w-96 */}
        ...
        {isTop5Open && ghanaData.top5_customers && (
          <SouthAfricaTop5 data={ghanaData.top5_customers} />
        )}
        </div>
        ...
        {isMissedRepaymentsOpen && ghanaData.missed_repayments_data && (
          <SouthAfricaMissedRepayment data={ghanaData.missed_repayments_data} />
        )}
        </div>
        ...
        {isStage2Open && ghanaData.top_20_stage2 && (
          <SouthAfricaStage2 data={ghanaData.top_20_stage2} />
        )}
        </div>
        ...
        {isSectorOpen && ghanaData.sector_data && (
          <SouthAfricaSector data={ghanaData.sector_data} />
        )}
      ...
      <div className='mt-8'>
        <FileUploadButton
          ...
          url="https://clr-1.onrender.com/southAfrica"
        />
      </div>
    </div>
  );
...
export default SouthAfricaScreen;
