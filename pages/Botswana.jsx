...
import BotswanaTop5 from "./components/Botswana/botswanaTop5";
import BotswanaMissedRepayment from "./components/Botswana/botswanaMissedRepayments";
import BotswanaStage2 from "./components/Botswana/botswanaStage2";
import BotswanaSector from "./components/Botswana/botswanaSector";    
...
const BotswanaScreen = () => {
  ...
  const loadPersistedData = async () => {
    const querySnapshot = await getDocs(collection(db, "botswanaData"));
  ...
  const handleDataLoaded = async (data) => {
    ...
    await setDoc(doc(db, "botswanaData", timestamp), relevantData);
  };

  return (
    <div className="bg-white min-h-screen max-w-6xl mx-auto mt-4 flex flex-col items-center justify-center p-4">
      <div className='flex mt-2 items-center text-center justify-center font-bold text-neutral-600 text-3xl'>
        Botswana's Credit Dashboard
      </div>
  ...
  {/* // md:w-1/2 lg:w-3/4 xl:w-full w-96 */}
        ...
        {isTop5Open && ghanaData.top5_customers && (
          <BotswanaTop5 data={ghanaData.top5_customers} />
        )}
        </div>
        ...
        {isMissedRepaymentsOpen && ghanaData.missed_repayments_data && (
          <BotswanaMissedRepayment data={ghanaData.missed_repayments_data} />
        )}
        </div>
        ...
        {isStage2Open && ghanaData.top_20_stage2 && (
          <BotswanaStage2 data={ghanaData.top_20_stage2} />
        )}
        </div>
        ...
        {isSectorOpen && ghanaData.sector_data && (
          <BotswanaSector data={ghanaData.sector_data} />
        )}
