...
const MozambiqueTop5 = ({ data }) => {
  ...
  const columns = React.useMemo(
    () => [
      ...
      { Header: 'Approved Facility Amount (Limit)', accessor: 'AMOUNT_FINANCED_USD', Cell: ({ value }) => formatNumber(value) },
      { Header: 'Total Exposures (USD)', accessor: 'OUTSTANDING_BALANCE_USD', Cell: ({ value }) => formatNumber(value) },
      { Header: 'IFRS', accessor: 'IFR CLASSIFICATION' },
  ...
  const downloadExcel = () => {
    const worksheetData = data.map(row => {
      return {
        ...
        'Sector': row.SECTOR,
        'Approved Facility Amount (Limit)': row['AMOUNT_FINANCED_USD'],
        'Total Exposures (USD)': row['OUTSTANDING_BALANCE_USD'],
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Mozambique Top 5');
    XLSX.writeFile(workbook, 'MozambiqueTop5.xlsx');
  };
