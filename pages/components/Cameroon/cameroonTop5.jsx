...
const CameroonTop5 = ({ data }) => {
  ...
  const columns = React.useMemo(
    () => [
      ...
      { Header: 'Total Exposures (USD)', accessor: 'OUTSANDING BALANCE USD', Cell: ({ value }) => formatNumber(value) },
      { Header: 'IFRS', accessor: 'IFRS_ CLASSIFICATION' },
      { Header: 'Classification', accessor: 'PRUDENTIAL_ CLASSIFICATION' },
    ],
    []
  );
  ...
  const downloadExcel = () => {
    const worksheetData = data.map(row => {
      return {
        ...
        'Total Exposures (USD)': row['OUTSANDING BALANCE USD'],
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Cameroon Top 5');
    XLSX.writeFile(workbook, 'CameroonTop5.xlsx');
  };
