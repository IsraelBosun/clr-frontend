...
const sectorLimits = {
  ...
  "Pharmaceutical": 4.7,
  "Transport & Logistics": 60.0,
  "Wood Processing and Distribution": 7.8,
  "Fintech": 0.0,
  "Brewery": 10.0
};

const CameroonSector = ({ data }) => {
  if (!data) {
    return <div className="p-4">No data available</div>;
  }

  // Calculate total outstanding balance
  const totalOutstandingBalance = data.reduce((total, item) => total + item["OUTSANDING BALANCE USD"], 0);

  // Add percentage, limit, and difference to the data
  const enhancedData = data.map((item, index) => {
    console.log(item, 'this is item')
    console.log([item["SECTOR "] ], 'CHECK HERE')
    const percentage = (item["OUTSANDING BALANCE USD"] / totalOutstandingBalance) * 100;
    const limit = sectorLimits[item.SECTOR] || 0;
    console.log(limit, 'this is it')
    console.log(sectorLimits[item.sector])
  ...
  const totalExposure = data.reduce((sum, item) => sum + item["OUTSANDING BALANCE USD"], 0);
  ...
  const columns = React.useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: "OUTSANDING BALANCE USD",
        Cell: ({ value }) => formatNumber(value),
      },
      {
        Header: "Percentage (%)",
  ...
  const downloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Cameroon Sector');
    XLSX.writeFile(workbook, 'CameroonSector.xlsx');
  };
