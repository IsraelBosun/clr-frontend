  ...
  const downloadExcel = () => {
    const worksheetData = data.map((row, index) => {
      return {
        ...
        "Approved Facility Amount ($)": row["APPROVED TOTAL FACILITY AMOUNT/LIMIT"],
        "Total Exposures ($)": row["TOTAL EXPOSURES(USD)"],
      };
    });

    const worksheet = XLSX.utils.json_to_sheet(worksheetData);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Stage 2 Loans');
    XLSX.writeFile(workbook, 'KenyaStage2.xlsx');
  };
