...
const CongoMissedRepayment = ({ data }) => {
  if (!data) {
  ...
  const downloadExcel = () => {
    const worksheetData = data.map((row, index) => {
      return {
        ...
        "Missed Installment ($)": row["TOTAL UNPAID"],
      };
    });
    ...
    XLSX.writeFile(workbook, 'CongoMissedRepayment.xlsx');
  };
...
export default CongoMissedRepayment;
