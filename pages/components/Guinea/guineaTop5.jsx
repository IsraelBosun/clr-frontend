...
const GuineaTop5 = ({ data }) => {
  ...
  const columns = React.useMemo(
    () => [
      ...
      { Header: 'Total Exposures (USD)', accessor: 'CURRENT EXPOSURE (USD)', Cell: ({ value }) => formatNumber(value) },
  ...
  const downloadExcel = () => {
    const worksheetData = data.map(row => {
      return {
        ...
        'Sector': row.SECTOR,
        'Approved Facility Amount (Limit)': row['APPROVED AMOUNT'],
        'Total Exposures (USD)': row['OUTSTANDING BALANCE'],
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Guinea Top 5');
    XLSX.writeFile(workbook, 'GuineaTop5.xlsx');
  };
