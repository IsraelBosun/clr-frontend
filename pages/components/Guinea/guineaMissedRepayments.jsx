...
const GuineaMissedRepayment = ({ data }) => {
  if (!data) {
  ...
  const downloadExcel = () => {
    const worksheetData = data.map((row, index) => {
      return {
        ...
        "Missed Installment ($)": row["UNPAID_AMOUNT_USD"],
      };
    });
    ...
    XLSX.writeFile(workbook, 'GuineaMissedRepayment.xlsx');
  };
...
export default GuineaMissedRepayment;
