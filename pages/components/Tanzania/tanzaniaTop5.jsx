...
const TanzaniaTop5 = ({ data }) => {
  ...
  const columns = React.useMemo(
    () => [
      ...
      { Header: 'Approved Facility Amount (Limit)', accessor: 'APPROVED AMOUNT TCY', Cell: ({ value }) => formatNumber(value) },
  ...
  const downloadExcel = () => {
    const worksheetData = data.map(row => {
      return {
        ...
        'Sector': row.SECTOR,
        'Approved Facility Amount (Limit)': row['APPROVED AMOUNT TCY'],
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Tanzania Top 5');
    XLSX.writeFile(workbook, 'TanzaniaTop5.xlsx');
  };
