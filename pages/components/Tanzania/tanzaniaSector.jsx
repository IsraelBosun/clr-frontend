...
const TanzaniaSector = ({ data }) => {
  ...
  const totalApprovedAmount = data.reduce((sum, item) => sum + item["APPROVED AMOUNT TCY"], 0);
  ...
  const downloadExcel = () => {
    const worksheetData = enhancedData.map((row, index) => ({
      "S/N": index + 1,
      "Sector": row.SECTOR,
      "Approved Facility Amount ($)": row["APPROVED AMOUNT TCY"],
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Tanzania Sector');
    XLSX.writeFile(workbook, 'TanzaniaSector.xlsx');
  };
