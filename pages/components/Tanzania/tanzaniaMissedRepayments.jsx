...
const TanzaniaMissedRepayment = ({ data }) => {
  if (!data) {
  ...
  const columns = React.useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: "UNPAID AMOUNT INTEREST(USD)",
        Cell: ({ value }) => formatNumber(value),
      },
    ],
    []
  ...
  const downloadExcel = () => {
    const worksheetData = data.map((row, index) => {
      return {
        ...
        "Missed Installment ($)": row["UNPAID AMOUNT INTEREST(USD)"],
      };
    });
    ...
    XLSX.writeFile(workbook, 'TanzaniaMissedRepayment.xlsx');
  };
...
export default TanzaniaMissedRepayment;
