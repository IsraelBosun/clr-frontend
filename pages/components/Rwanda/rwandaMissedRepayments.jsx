...
const RwandaMissedRepayment = ({ data }) => {
  if (!data) {
  ...
  const downloadExcel = () => {
    const worksheetData = data.map((row, index) => {
      return {
        ...
        "Missed Installment ($)": row["UNPAID AMOUNT"],
      };
    });
    ...
    XLSX.writeFile(workbook, 'RwandaMissedRepayment.xlsx');
  };
...
export default RwandaMissedRepayment;
