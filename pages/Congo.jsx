...
import CongoStage2 from "./components/Congo/congoStage2";
import CongoSector from "./components/Congo/congoSector";    
...
const CongoScreen = () => {
  ...
  const loadPersistedData = async () => {
    const querySnapshot = await getDocs(collection(db, "congoData"));
  ...
  {/* // md:w-1/2 lg:w-3/4 xl:w-full w-96 */}
        ...
        {isTop5Open && ghanaData.top5_customers && (
          <CongoTop5 data={ghanaData.top5_customers} />
        )}
        </div>
        ...
        {isMissedRepaymentsOpen && ghanaData.missed_repayments_data && (
          <CongoMissedRepayment data={ghanaData.missed_repayments_data} />
        )}
        </div>
        ...
        {isStage2Open && ghanaData.top_20_stage2 && (
          <CongoStage2 data={ghanaData.top_20_stage2} />
        )}
        </div>
        ...
        {isSectorOpen && ghanaData.sector_data && (
          <CongoSector data={ghanaData.sector_data} />
        )}
      ...
      <div className='mt-8'>
        <FileUploadButton
          ...
          url="https://clr-1.onrender.com/congo"
        />
      </div>
    </div>
  );
