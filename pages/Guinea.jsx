...
import GuineaStage2 from "./components/Guinea/guineaStage2";
import GuineaSector from "./components/Guinea/guineaSector";
...
const GuineaScreen = () => {
  ...
  const metrics = [
    ...
    {
      label: "FCY Direct Percentage",
      key: "fcy_direct_percentage",
      isPercentage: true,
    },
    {
      label: "FCY Total Percentage",
      key: "fcy_total_percentage",
      isPercentage: true,
    },
    {
      label: "Percentage Of Top 5",
      key: "percentage_of_top5",
      isPercentage: true,
    },
  ...
  const loadPersistedData = async () => {
    const querySnapshot = await getDocs(collection(db, "guineaData"));
  ...
  const handleDataLoaded = async (data) => {
    ...
    await addDoc(collection(db, "guineaData"), relevantData);
  };
    ...
    <div className="bg-white min-h-screen max-w-6xl mx-auto mt-4 flex flex-col items-center justify-center p-4">
      ...
      <div className="w-full sm:w-96 md:w-1/2 lg:w-3/4 xl:w-full w-96 overflow-x-auto m-4">
        {isTop5Open && ghanaData.top5_customers && (
          <GuineaTop5 data={ghanaData.top5_customers} />
        )}
      </div>
      ...
      <div className="w-full sm:w-96 md:w-1/2 lg:w-3/4 xl:w-full w-96 overflow-x-auto m-4">
        {isMissedRepaymentsOpen && ghanaData.missed_repayments_data && (
          <GuineaMissedRepayment data={ghanaData.missed_repayments_data} />
        )}
      </div>
      ...
      <div className="w-full sm:w-96 md:w-1/2 lg:w-3/4 xl:w-full w-96 overflow-x-auto m-4">
        {isStage2Open && ghanaData.top_20_stage2 && (
          <GuineaStage2 data={ghanaData.top_20_stage2} />
        )}
      </div>
      ...
      <div className="w-full sm:w-96 md:w-1/2 lg:w-3/4 xl:w-full w-96 overflow-x-auto m-4">
        {isSectorOpen && ghanaData.sector_data && (
          <GuineaSector data={ghanaData.sector_data} />
        )}
      </div>
      ...
      <div className="mt-8">
        <FileUploadButton
          ...
          url="https://clr-1.onrender.com/guinea"
        />
      </div>
    </div>
  );
