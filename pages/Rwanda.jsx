...
import RwandaTop5 from "./components/Rwanda/rwandaTop5";
import RwandaMissedRepayment from "./components/Rwanda/rwandaMissedRepayments";
import RwandaStage2 from "./components/Rwanda/rwandaStage2";
import RwandaSector from "./components/Rwanda/rwandaSector";    
...
const RwandaScreen = () => {
  ...
  const loadPersistedData = async () => {
    const querySnapshot = await getDocs(collection(db, "rwandaData"));
  ...
  const handleDataLoaded = async (data) => {
    ...
    await setDoc(doc(db, "rwandaData", timestamp), relevantData);
  };
  ...
  {/* // md:w-1/2 lg:w-3/4 xl:w-full w-96 */}
        ...
        {isTop5Open && ghanaData.top5_customers && (
          <RwandaTop5 data={ghanaData.top5_customers} />
        )}
        </div>
        ...
        {isMissedRepaymentsOpen && ghanaData.missed_repayments_data && (
          <RwandaMissedRepayment data={ghanaData.missed_repayments_data} />
        )}
        </div>
        ...
        {isStage2Open && ghanaData.top_20_stage2 && (
          <RwandaStage2 data={ghanaData.top_20_stage2} />
        )}
        </div>
        ...
        {isSectorOpen && ghanaData.sector_data && (
          <RwandaSector data={ghanaData.sector_data} />
        )}
