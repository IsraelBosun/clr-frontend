...
import GambiaTop5 from "./components/Gambia/gambiaTop5";
import GambiaMissedRepayment from "./components/Gambia/gambiaMissedRepayments";
import GambiaStage2 from "./components/Gambia/gambiaStage2";
import GambiaSector from "./components/Gambia/gambiaSector";    
...
const GambiaScreen = () => {
  ...
  const loadPersistedData = async () => {
    const querySnapshot = await getDocs(collection(db, "gambiaData"));
  ...
  const handleDataLoaded = async (data) => {
    ...
    await setDoc(doc(db, "gambiaData", timestamp), relevantData);
  };

  return (
    <div className="bg-white min-h-screen max-w-6xl mx-auto mt-4 flex flex-col items-center justify-center p-4">
      <div className='flex items-center mt-2 text-center justify-center font-bold text-neutral-600 text-3xl'>
        Gambia's Risk Dashboard
      </div>
  ...
  {/* // md:w-1/2 lg:w-3/4 xl:w-full w-96 */}
        ...
        {isTop5Open && ghanaData.top5_customers && (
          <GambiaTop5 data={ghanaData.top5_customers} />
        )}
        </div>
        ...
        {isMissedRepaymentsOpen && ghanaData.missed_repayments_data && (
          <GambiaMissedRepayment data={ghanaData.missed_repayments_data} />
        )}
        </div>
        ...
        {isStage2Open && ghanaData.top_20_stage2 && (
          <GambiaStage2 data={ghanaData.top_20_stage2} />
        )}
        </div>
        ...
        {isSectorOpen && ghanaData.sector_data && (
          <GambiaSector data={ghanaData.sector_data} />
        )}
      ...
      <div className='mt-8'>
        <FileUploadButton
          ...
          url="https://clr-1.onrender.com/gambia"
        />
      </div>
    </div>
  );
